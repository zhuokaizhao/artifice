...
def conv_upsample(inputs,
	...
	if mask is None:
		inputs = keras.layers.Conv2DTranspose(filters, size,
												strides=size,
												padding='same',
...
class BetterSparseUNet(SparseUNet):
	...
	def __init__(self, **kwargs):
		...
		self.level_input_tile_sizes = self.compute_level_input_shapes_(self.base_shape, self.num_levels, self.level_depth)

		# block sizes
		self.level_input_block_size = list(2 * np.array(self.block_size))
		self.level_output_block_size = list(np.array(self.level_input_block_size) - 2 * self.level_depth)
	...
	def forward(self, inputs):
		...
		bin_counts, active_block_indices = lay.ReduceMask(block_size=self.block_size,
		...
		for i, filters in enumerate(self.level_filters[1:]):
			level = i + 1
			blocks = lay.SparseGather(block_size=self.block_size,
									block_stride=self.block_stride)(
		...
		dropped = keras.layers.Dropout(rate=self.dropout)(cropped)
		blocked = lay.SparseGather(block_size=self.level_input_block_size,
		...
		inputs = lay.SparseScatter([self.batch_size] + self.output_tile_shapes[level] + [blocks.shape[3]],
		...
		mask_blocks = conv(blocks,
							1,
							kernel_shape=[1, 1],
							activation=None,
							norm=False,
							padding='same')
		mask = lay.SparseScatter([self.batch_size] + self.output_tile_shapes[level] + [1],
								block_size=self.level_output_block_size,
								block_stride=self.level_output_block_stride,
...
class AutoSparseUNet(BetterSparseUNet):
	...
	def forward(self, inputs):
		...
		for i, filters in enumerate(self.level_filters[1:]):
			...
			bin_couns, active_block_indices = lay.ReduceMask(block_size=self.block_size,
															block_stride=self.block_stride,
															tol=self.tol)(mask)
